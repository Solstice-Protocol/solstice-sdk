 ...
 * 
 * These tests verify the complete SDK functionality
 * Note: Requires Solana devnet connection and test circuits
 */

import { SolsticeSDK } from '../src/core/SolsticeSDK';
import { PublicKey } from '@solana/web3.js';

// Mock Solana Web3.js before importing SDK
jest.mock('@solana/web3.js', () => {
  ...
  return {
    ...
    PublicKey: {
      ...originalModule.PublicKey,
      findProgramAddressSync: jest.fn().mockReturnValue([
        new originalModule.PublicKey('11111111111111111111111111111112'),
        255
      ]),
    },
  };
...
// Mock QR data (simplified for testing)
const mockQRData = `<uid uid="xxxx-xxxx-xxxx" name="Test User" dob="01/01/1990" gender="M" co="Father Name" dist="Test District" state="Test State" pc="123456" />`;
...
describe('SolsticeSDK Integration Tests', () => {
  ...
  describe('SDK Initialization', () => {
    ...
    test('should have correct configuration', () => {
      ...
      expect(config.programId.toString()).toBe('8jrTVUyvHrL5WTWyDoa6PTJRhh3MwbvLZXeGT81YjJjz');
    });
  });

  describe('Wallet Connection', () => {
    test('should connect wallet successfully', async () => {
      await expect(sdk.connect(mockWalletAdapter as any)).resolves.not.toThrow();
    });
  ...
  describe('Identity Management', () => {
    test('should register identity', async () => {
      const txSignature = await sdk.registerIdentity(mockQRData);
